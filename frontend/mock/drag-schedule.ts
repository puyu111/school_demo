...
export default {
  ...
  'PUT /api/courses/:id': (req: any, res: any) => {
    const { id } = req.params;
    const index = mockCourses.list.findIndex((item: Course) => item.id === Number(id));
  ...
  'GET /api/classrooms': (req: any, res: any) => {
    ...
    // 楼栋筛选
    if (building) {
      data = data.filter((item: Classroom) => item.building.includes(building as string));
    }
    
    // 可用性筛选
    if (available !== undefined) {
      data = data.filter((item: Classroom) => item.available === (available === 'true'));
    }
    
    // 容量筛选
    if (capacityMin) {
      data = data.filter((item: Classroom) => item.capacity >= Number(capacityMin));
    }
    
    if (capacityMax) {
      data = data.filter((item: Classroom) => item.capacity <= Number(capacityMax));
    }
  ...
  'GET /api/schedules': (req: any, res: any) => {
    ...
    // 教师筛选
    if (teacherId) {
      data = data.filter((item: Schedule) => item.teacher.id === Number(teacherId));
    }
    
    // 课程筛选
    if (courseId) {
      data = data.filter((item: Schedule) => item.course.id === Number(courseId));
    }
    
    // 教室筛选
    if (classroomId) {
      data = data.filter((item: Schedule) => item.classroom.id === Number(classroomId));
    }
  ...
  'POST /api/schedules': (req: any, res: any) => {
    const { courseId, teacherId, classroomId, weekday, timeSlotId, weekRange } = req.body;
    
    // 模拟冲突检测
    const conflict = mockSchedules.some((schedule: Schedule) => 
    ...
    // 查找相关数据
    const course = mockCourses.list.find((c: Course) => c.id === Number(courseId));
    const teacher = mockTeachers.list.find((t: Teacher) => t.id === Number(teacherId));
    const classroom = mockClassrooms.list.find((c: Classroom) => c.id === Number(classroomId));
    const timeSlot = [
      { id: 1, start: '08:00', end: '09:40', period: '1-2节' },
      { id: 2, start: '10:00', end: '11:40', period: '3-4节' },
      { id: 3, start: '14:00', end: '15:40', period: '5-6节' },
  ...
  // 6. 自动排课接口
  'POST /api/schedules/auto-arrange': async (req: { body: { courses: any[], constraints?: any } }, res: any): Promise<void> => {
    ...
    // 生成排课结果
    const arrangedCourses = courses.slice(0, 5).map((course: any, index: number) => {
      const teacher = mockTeachers.list[index % mockTeachers.list.length];
      const classroom = mockClassrooms.list[index % mockClassrooms.list.length];
      const weekdays = ['周一', '周二', '周三', '周四', '周五'];
      const timeSlots = [
        { id: 1, start: '08:00', end: '09:40', period: '1-2节' },
        { id: 2, start: '10:00', end: '11:40', period: '3-4节' },
    ...
    res.json({
      ...
      data: {
        ...
        stats: {
          ...
          successRate: ((arrangedCourses.length - conflicts.length) / arrangedCourses.length * 100).toFixed(2) + '%',
        },
  ...
  'POST /api/schedules/check-conflict': (req: any, res: any) => {
    ...
    // 检查教室冲突
    const classroomConflict = mockSchedules.find((schedule: Schedule) => 
    ...
    // 检查教师冲突
    const teacherConflict = mockSchedules.find((schedule: Schedule) => 
    ...
    // 检查课程间隔（同一课程不能连续安排）
    const intervalConflict = mockSchedules.find((schedule: Schedule) => 
  ...
  'POST /api/schedules/batch': (req: any, res: any) => {
    ...
    if (action === 'delete') {
      mockSchedules = mockSchedules.filter((schedule: Schedule) => 
      ...
      const updated = mockSchedules.filter((schedule: Schedule) => 
        scheduleIds.includes(schedule.scheduleId)
  ...
  'GET /api/stats': (req: any, res: any) => {
    const stats = {
      ...
      courses: {
        ...
        byType: {
          必修: mockCourses.list.filter((c: Course) => c.type === '必修').length,
          选修: mockCourses.list.filter((c: Course) => c.type === '选修').length,
          限选: mockCourses.list.filter((c: Course) => c.type === '限选').length,
        },
        byDepartment: {},
      },
      ...
      teachers: {
        ...
        byTitle: {
          教授: mockTeachers.list.filter((t: Teacher) => t.title === '教授').length,
          副教授: mockTeachers.list.filter((t: Teacher) => t.title === '副教授').length,
          讲师: mockTeachers.list.filter((t: Teacher) => t.title === '讲师').length,
          助教: mockTeachers.list.filter((t: Teacher) => t.title === '助教').length,
        },
        workload: {
          average: mockTeachers.list.reduce((sum: number, t: Teacher) => sum + t.workload, 0) / mockTeachers.list.length,
      ...
      schedules: {
        ...
        byStatus: {
          已安排: mockSchedules.filter((s: Schedule) => s.status === '已安排').length,
          待审核: mockSchedules.filter((s: Schedule) => s.status === '待审核').length,
          冲突: mockSchedules.filter((s: Schedule) => s.status === '冲突').length,
          已取消: mockSchedules.filter((s: Schedule) => s.status === '已取消').length,
        },
        byWeekday: {
          周一: mockSchedules.filter((s: Schedule) => s.weekday === '周一').length,
          周二: mockSchedules.filter((s: Schedule) => s.weekday === '周二').length,
          周三: mockSchedules.filter((s: Schedule) => s.weekday === '周三').length,
          周四: mockSchedules.filter((s: Schedule) => s.weekday === '周四').length,
          周五: mockSchedules.filter((s: Schedule) => s.weekday === '周五').length,
        },
        conflictRate: (mockSchedules.filter((s: Schedule) => s.conflict).length / mockSchedules.length * 100).toFixed(2) + '%',
      },
      
      // 教室使用率
      classrooms: {
        total: mockClassrooms.list.length,
        available: mockClassrooms.list.filter((c: Classroom) => c.available).length,
        byType: {
          多媒体: mockClassrooms.list.filter((c: Classroom) => c.type === '多媒体').length,
          实验室: mockClassrooms.list.filter((c: Classroom) => c.type === '实验室').length,
          普通: mockClassrooms.list.filter((c: Classroom) => c.type === '普通').length,
          计算机房: mockClassrooms.list.filter((c: Classroom) => c.type === '计算机房').length,
        },
        averageCapacity: mockClassrooms.list.reduce((sum: number, c: Classroom) => sum + c.capacity, 0) / mockClassrooms.list.length,
      },
      ...
      realtime: {
        todaySchedules: mockSchedules.filter((s: Schedule) => {
          ...
          const weekdayMap: { [key: string]: number } = { '周一': 1, '周二': 2, '周三': 3, '周四': 4, '周五': 5 };
          return weekdayMap[s.weekday] === today;
        }).length,
        pendingApprovals: mockSchedules.filter((s: Schedule) => s.status === '待审核').length,
        activeConflicts: mockSchedules.filter((s: Schedule) => s.conflict).length,
      },
    };
  ...
  'GET /api/drag-schedule/time-slots': (req: any, res: any) => {
    res.json({
      ...
      data: {
        ...
        timeSlots: [
          { id: 'slot-1', label: '第 1 节', startTime: '08:00', endTime: '08:45', duration: 45, halfDayType: 'morning', isBreak: false, breakAfter: 10, isSchedulable: true },
          { id: 'slot-2', label: '第 2 节', startTime: '08:50', endTime: '09:35', duration: 45, halfDayType: 'morning', isBreak: false, breakAfter: 15, isSchedulable: true },
          { id: 'slot-3', label: '第 3 节', startTime: '09:50', endTime: '10:35', duration: 45, halfDayType: 'morning', isBreak: false, breakAfter: 10, isSchedulable: true },
          { id: 'slot-4', label: '第 4 节', startTime: '10:45', endTime: '11:30', duration: 45, halfDayType: 'morning', isBreak: false, breakAfter: 10, isSchedulable: true },
          { id: 'slot-5', label: '第 5 节', startTime: '14:00', endTime: '14:45', duration: 45, halfDayType: 'afternoon', isBreak: false, breakAfter: 10, isSchedulable: true },
          { id: 'slot-6', label: '第 6 节', startTime: '14:50', endTime: '15:35', duration: 45, halfDayType: 'afternoon', isBreak: false, breakAfter: 10, isSchedulable: true },
          { id: 'slot-7', label: '第 7 节', startTime: '15:50', endTime: '16:35', duration: 45, halfDayType: 'afternoon', isBreak: false, breakAfter: 10, isSchedulable: true },
          { id: 'slot-8', label: '第 8 节', startTime: '16:45', endTime: '17:30', duration: 45, halfDayType: 'afternoon', isBreak: false, breakAfter: 10, isSchedulable: true },
          { id: 'slot-9', label: '第 9 节', startTime: '19:00', endTime: '19:45', duration: 45, halfDayType: 'evening', isBreak: false, breakAfter: 10, isSchedulable: true },
          { id: 'slot-10', label: '第 10 节', startTime: '19:50', endTime: '20:35', duration: 45, halfDayType: 'evening', isBreak: false, breakAfter: 10, isSchedulable: true },
        ],
        dailyConfig: {
          totalPeriods: 10,
          defaultDuration: 45,
  ...
  'PUT /api/drag-schedule/time-slots': (req: any, res: any) => {
    res.json({
      ...
      data: {
        ...
        dailyConfig: req.body.dailyConfig || { totalPeriods: 10, defaultDuration: 45, defaultBreakDuration: 10 },
  ...
  'POST /api/drag-schedule/time-slots/reset': (req: any, res: any) => {
    res.json({
      ...
      message: '配置已重置为默认值',
      data: {
        dailyConfig: { totalPeriods: 10, defaultDuration: 45, defaultBreakDuration: 10 },
        timeSlots: [
          { id: 'slot-1', label: '第 1 节', startTime: '08:00', endTime: '08:45', duration: 45, halfDayType: 'morning', isBreak: false, breakAfter: 10, isSchedulable: true },
        ],
        halfDayConfigs: [
  ...
  'GET /api/drag-schedule/conflicts/types': (req: any, res: any) => {
    res.json({
      ...
      message: 'success',
      data: [
        { type: 'teacher', name: '教师冲突', description: '同一教师在同一时间有多个课程安排' },
        { type: 'room', name: '教室冲突', description: '同一教室在同一时间被多个课程占用' },
        { type: 'class', name: '班级冲突', description: '同一班级在同一时间有多个课程' },
        { type: 'duration', name: '时长冲突', description: '课程时长超出一天的可排课时间范围' },
      ],
      timestamp: Date.now(),
    });
  },
